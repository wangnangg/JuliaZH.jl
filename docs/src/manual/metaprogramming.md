...
在Julia语言中，对元编程的支持，是继承自Lisp语言的最强大遗产。类似Lisp，Julia自身的代码也是语言本身的数据结构。由于代码是由这门语言本身所构造和处理的对象所表示的，因此程序也可以转换和生成自身语言的代码。因此，无需额外的构建步骤，我们就能完成复杂的代码生成，并且在抽象语法树 ([abstract syntax trees](https://en.wikipedia.org/wiki/Abstract_syntax_tree)) 层面对代码进行变换。作为对比，基于预处理器的宏（比如C和C++的宏）是通过在代码被编译或解释之前进行文本层面的处理和替换实现的。因为Julia中的数据类型和代码都是通过Julia自身的数据结构来表示的，所以反射 ([reflection](https://en.wikipedia.org/wiki/Reflection_%28computer_programming%29)) 可以用来获取程序的类型信息。
...
接下来这个字符串就会被解析（[parse](https://en.wikipedia.org/wiki/Parsing#Computer_languages)）成一个表达式（expression）对象，它的类型为`Expr`：
...
- 一个`Symbol`，代表该表达式的种类。一个符号（symbol）就是一个驻留字符串（[interned string](https://en.wikipedia.org/wiki/String_interning)）标识符。
...
表达式也可以通过前缀表示（[prefix notation](https://en.wikipedia.org/wiki/Polish_notation)）直接构造出来：
...
**这里想要说明的其实就是Julia的代码是用Julia语言本身的数据结构表示的。**
...
`Meta.show_sexpr`可以把`Expr`用S表达式（[S-expression](https://en.wikipedia.org/wiki/S-expression)）的形式打印出来，用过Lisp的人应该很熟。
下面是一个例子：
...
冒号`:`在Julia中有两个语法层面的作用。第一个作用是构造一个[`Symbol`](@ref)对象。一个`Symbol`对象就是一个驻留字符串，
它是表达式的基本组成之一：
...
[`Symbol`](@ref)的构造函数接受任意数量的参数。构造函数把这些参数的字符串表示连接在一起构造成一个`Symbol`：
...
在一个表达式中，符号指代着变量；当一个表达式被求值时，符号会被替换成它在当前作用域中所指代的变量的值。
...
冒号`:`在语法层面的第二个作用是用来快速构造`Expr`，这叫做*引用*（*quoting*）。冒号`:`加上一句被括起来的Julia代码就可以构造出代表该语句的`Expr`对象：
...
（可以用`ex.head`和`ex.args`，[`dump`](@ref)或者[`Meta.@dump`](@ref)来查看以上表达式的结构）
...
前述表达式也可以用[`Meta.parse`](@ref)或者`Expr`的构造函数来构造：
...
引用的另一种语法是通过“引用块”实现多重表达式： 在引用块里，代码被包含在quote ... end中。 
...
用参数值直接构建表达式对象，这种方法是非常强大的, 但是与“正常”的Julia语法相比， `Expr` 对象构建器就可能显得冗长。作为另一个选择， Julia允许, 把字面值或者表达式对象 “拼接 (splicing) ” 或者“内插” 进 一个被引用的表达式语句（即表达式对象）。 内插的内容之前加 `$` 前缀.
...
对于没有被引用（quote）的表达式语句，是不能做内插的。如果对这种表达式语句做内插操作，将会导致编译错误 (compile-time error)。
...
在这个例子里, tuple `(1,2,3)` 作为一个表达式语句，先被 `:` 引用成了表达式对象，再被内插进一个条件测试：
...
使用符号`$`来进行表达式内插是有意设计的结果。这让表达式内插，字符串内插（[string interpolation](@ref string-interpolation)）和指令内插（[command interpolation](@ref command-interpolation)）联系了起来。表达式内插语法使得简洁易读的表达式构造成为了可能。
...
使用`$`的内插语法只允许插入单个表达式。如果想把一个数组的表达式展开作为另一个表达式的参数，那么需要使用`$(xs...)`语法。见下面的例子：
...
引用（quote）表达式允许嵌套使用，但要正确地理解内插的工作原理需要一点技巧。考虑下面这个例子：
...
可以看到，最终生成的表示包含`Expr(:$, :x)`，这意味着`x`还没有被求值。换句话说，`$x`是属于内层的引用（quote）表达式的，它只有当外层引用（quote）被求值之后才会被展开：
...
现在`:(1 + 2)`出现在结果中，替换了原来的`:x`。这时候对整个表达式求值会直接得到一个内插的`3`。
...
这里每个`$`就相当一次求值：一个`$`类似于`eval(:x)`，其结果是`x`本身，那么两个`$`就等于`eval(eval(:x))`。
...
一般来说，`quote`在抽象语法树（AST）中是一个`head`为`:quote`的`Expr`：
...
我们已经知道，这些引用表达式支持用`$`内插。然而，有时候我们不想表达式里的内插被展开，这时候需要使用`QuoteNode`，但是Julia中暂时还没有相应的语法支持构造这种表达式。解析器解析单个符号这种简单的引用（quote）时会产生`QuoteNode`：
